...
public class MiningEnchantmentHelper {
        ...
        for (int i = 0; i < nbtList.size(); i++) {
            ...
            Registries.ENCHANTMENT.getOrEmpty(EnchantmentHelper.getIdFromNbt(compound)).ifPresent(enchantment -> {
                if (enchantment instanceof MiningEnchantment) {
                    consumer.accept((MiningEnchantment) enchantment, stack, EnchantmentHelper.getLevelFromNbt(compound));
                }
            });
        }
    }
    ...
    @FunctionalInterface
    interface Consumer {
        void accept(MiningEnchantment enchantment, ItemStack stack, int level);
    }
}